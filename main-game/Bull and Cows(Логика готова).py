...
def main_game_hard(start_time):
    ...
    while True:
            ...
            if flag:
                ...
                count += 1
                if count == 7:
                    return print(f'Вы исчерпали количество попыток, хотите попробовать еще раз?, Число загаданное компьютером: {mechanical_digits}')
                if bulls != 5:
                    print(f'Коровы: {cows}')
                    print(f'Быки: {bulls}')
                    print(f'Попыток: {count}')
...
def change_the_difficulty():
    ...
    if user_changer == 'Лёгкий':
        main_game_easy(start_time)
    if user_changer == 'Средний':
        main_game_medium(start_time)
    if user_changer == 'Сложный':
